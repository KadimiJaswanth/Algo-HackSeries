      ...
      {mode === "find" ? (
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="text-right text-sm text-muted-foreground">
                        <p>{ride.route.distance} • {ride.route.duration}</p>
                      </div>
                    </div>
                    ...
                    {ride.passengers.length > 0 && (
                      <div>
                        <p className="text-sm font-medium mb-2">Fellow Travelers:</p>
                        <div className="flex items-center space-x-2">
                          {ride.passengers.map((passenger, index) => (
                            <div key={index} className="flex items-center space-x-2 glass px-2 py-1 rounded-md">
                              ...
                              <div className="flex items-center">
                                ...
                                <span className="text-xs ml-1">{passenger.rating}</span>
                              </div>
                            </div>
                          ))}
                        </div>
                    ...
                    <div className="flex items-center justify-between pt-2 border-t border-border/50">
                      <div className="flex items-center space-x-2">
                        ...
                        <span className="text-sm text-green-400">Carbon saved: {ride.carbonSavings}</span>
                      </div>
                      <Button 
                        size="sm"
                        className="glow"
        ...
        <div className="space-y-6">
          ...
          <Card className="glass">
            <CardHeader>
              <CardTitle className="text-gradient">Potential Earnings</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div className="text-center glass p-4 rounded-lg">
                  <DollarSign className="h-6 w-6 mx-auto mb-2 text-green-400" />
                  <p className="text-2xl font-bold text-green-400">${offerRideData.estimatedEarnings}</p>
                  <p className="text-sm text-muted-foreground">Est. earnings</p>
                </div>
                <div className="text-center glass p-4 rounded-lg">
                  <Users className="h-6 w-6 mx-auto mb-2 text-blue-400" />
                  <p className="text-2xl font-bold text-blue-400">{offerRideData.potentialPassengers}</p>
                  <p className="text-sm text-muted-foreground">Interested riders</p>
                </div>
                <div className="text-center glass p-4 rounded-lg">
                  <Route className="h-6 w-6 mx-auto mb-2 text-purple-400" />
                  <p className="text-2xl font-bold text-purple-400">{offerRideData.routeOptimization}</p>
                  <p className="text-sm text-muted-foreground">Route efficiency</p>
                </div>
                <div className="text-center glass p-4 rounded-lg">
                  <Leaf className="h-6 w-6 mx-auto mb-2 text-green-400" />
                  <p className="text-xl font-bold text-green-400">{offerRideData.carbonImpact}</p>
          ...
          <Card className="glass">
            <CardHeader>
              <CardTitle className="text-gradient">Pool Ride Benefits</CardTitle>
            </CardHeader>
            <CardContent>
